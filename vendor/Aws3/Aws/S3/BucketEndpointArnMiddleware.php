...
use Webinista\WreadIt\Aws3\Aws\Arn\AccessPointArnInterface;
use Webinista\WreadIt\Aws3\Aws\Arn\ArnParser;
use Webinista\WreadIt\Aws3\Aws\Arn\ObjectLambdaAccessPointArn;
use Webinista\WreadIt\Aws3\Aws\Arn\Exception\InvalidArnException;
use Webinista\WreadIt\Aws3\Aws\Arn\AccessPointArn as BaseAccessPointArn;
use Webinista\WreadIt\Aws3\Aws\Arn\S3\OutpostsAccessPointArn;
use Webinista\WreadIt\Aws3\Aws\Arn\S3\MultiRegionAccessPointArn;
use Webinista\WreadIt\Aws3\Aws\Arn\S3\OutpostsArnInterface;
...
use Webinista\WreadIt\Aws3\Aws\Exception\UnresolvedEndpointException;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(BaseAccessPointArn $arn, RequestInterface $req)
    {
        ...
        $region = \Webinista\WreadIt\Aws3\Aws\strip_fips_pseudo_regions($region);
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!$clientPart->isRegionMatch($this->region, 's3')) {
                $clientPart = $this->partitionProvider->getPartition(\Webinista\WreadIt\Aws3\Aws\strip_fips_pseudo_regions($this->region), 's3');
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
