...
use Webinista\WreadIt\Aws3\Aws\Api\Parser\PayloadParserTrait;
...
use Webinista\WreadIt\Aws3\Aws\S3\Exception\PermanentRedirectException;
use Webinista\WreadIt\Aws3\Aws\S3\Exception\S3Exception;
use Webinista\WreadIt\Aws3\GuzzleHttp\Promise\PromiseInterface;
use Webinista\WreadIt\Aws3\GuzzleHttp\Promise\RejectedPromise;
...
trait S3ClientTrait
{
    ...
    public function deleteMatchingObjectsAsync($bucket, $prefix = '', $regex = '', array $options = [])
    {
        ...
        if ($regex) {
            $iter = \Webinista\WreadIt\Aws3\Aws\filter($iter, function ($c) use ($regex) {
