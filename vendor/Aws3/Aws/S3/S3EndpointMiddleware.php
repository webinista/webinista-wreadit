...
use Webinista\WreadIt\Aws3\GuzzleHttp\Exception\InvalidArgumentException;
...
class S3EndpointMiddleware
{
    ...
    private function applyPathStyleEndpointCustomizations(CommandInterface $command, RequestInterface $request)
    {
        if ($command->getName() == 'WriteGetObjectResponse') {
            ...
            $fips = \Webinista\WreadIt\Aws3\Aws\is_fips_pseudo_region($this->region) ? "-fips" : "";
            $region = \Webinista\WreadIt\Aws3\Aws\strip_fips_pseudo_regions($this->region);
